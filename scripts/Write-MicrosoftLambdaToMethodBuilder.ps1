...
function Write-MicrosoftLambdaToMethodBuilder {
    ...
    # Prefer Microsoft's actual LambdaExpression -> MethodBuilder compiler path.
    # This initializes signature, closure state, IL generator, return labels,
    # bound constants, and compiler scope against the MethodBuilder itself.
    $compileToMethod = $compiler.GetMethods(
        [Reflection.BindingFlags]'Static,Public,NonPublic') |
        Where-Object {
            $p = $_.GetParameters()
            $_.Name -eq 'Compile' -and
            $p.Count -eq 2 -and
            $p[0].ParameterType -eq [Linq.Expressions.LambdaExpression] -and
            $p[1].ParameterType -eq [Reflection.Emit.MethodBuilder]
        } |
        Select-Object -First 1

    if($null -ne $compileToMethod){
        $null = $compileToMethod.Invoke(
            $null,
            [object[]]@($Lambda,$MethodBuilder)
        )
        return
    }
