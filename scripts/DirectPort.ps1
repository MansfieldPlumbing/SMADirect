...
if ($isDirectExecution) {
    if ($Headless) { exit 0 }
    $canvas = New-DirectPortCanvas -Width $Width -Height $Height -Title $Title -SoftwareRendering:$SoftwareRendering -EnableSharing:$EnableSharing
    $draw = { param($state) [void]$canvas.BeginDraw([uint32]4278782994); $canvas.DrawText($Title,24,24,[single]([Math]::Max(1,$state.Width-48)),64,[uint32]4294967295,24,'Segoe UI',$true,0); [void]$canvas.Present() }
    try {
        $state=$canvas.Pump(0); & $draw $state; $lastRedraw=$state.RedrawSerial; $lastResize=$state.ResizeSerial
        while ($canvas.Alive) {
            $state=$canvas.WaitEvent(); if($null -eq $state){break}
            if($state.RedrawSerial -ne $lastRedraw -or $state.ResizeSerial -ne $lastResize){ $lastRedraw=$state.RedrawSerial; $lastResize=$state.ResizeSerial; & $draw $state }
        }
