...
param(
    [switch] $Headless,
    [string] $DirectPortPath
)

# DirectPort bootstrap is application-generic path discovery. It does not depend
# on the current working directory and does not create an application runtime.
$calculatorHeadless = [bool]$Headless

function Write-DirectPortPanel {
...
function Wait-DirectPortFailureAcknowledge {
...
$explicitOverride = $PSBoundParameters.ContainsKey('DirectPortPath')
$callerOverride = $null
if (-not $explicitOverride) {
    $callerVariable = Get-Variable -Name DIRECTPORT_PATH -ErrorAction SilentlyContinue
    if ($null -ne $callerVariable) { $callerOverride = [string]$callerVariable.Value }
}

$dpCandidates = [Collections.Generic.List[string]]::new()
if ($explicitOverride) { $dpCandidates.Add($DirectPortPath) }
elseif (-not [string]::IsNullOrWhiteSpace($callerOverride)) { $dpCandidates.Add($callerOverride) }
elseif (-not [string]::IsNullOrWhiteSpace($env:DIRECTPORT_PATH)) { $dpCandidates.Add($env:DIRECTPORT_PATH) }
else {
    $dpCandidates.Add((Join-Path $PSScriptRoot 'DirectPort.ps1'))
    $dpCandidates.Add((Join-Path (Split-Path $PSScriptRoot -Parent) 'DirectPort.ps1'))
}
...
if (-not $dpPath) {
    $kind = if ($explicitOverride) { 'Explicit -DirectPortPath is invalid.' } else { 'DirectPort.ps1 was not found.' }
    throw "$kind Attempted: $($dpAttempts -join '; ')"
}

try {
    $attachmentContext = Get-Variable -Name DirectPortApplicationContext -Scope Global -ValueOnly -ErrorAction SilentlyContinue
    if ($null -ne $attachmentContext -and $attachmentContext.AttachedToExistingDirectPort -and $null -eq $attachmentContext.DirectPort) {
        throw "Calculator requires owner dispatch for DirectPort capabilities from execution placement '$($attachmentContext.ExecutionPlacement)'."
    }
    if ($null -ne $attachmentContext -and $null -ne $attachmentContext.DirectPort) {
        $loadedRoot = $attachmentContext.DirectPort
        $global:DirectPort = $loadedRoot
        $dpPath = [IO.Path]::GetFullPath([string]$attachmentContext.DirectPortPath)
    } else {
        $loadedRoot = Get-Variable -Name DirectPort -Scope Global -ValueOnly -ErrorAction SilentlyContinue
    }
    if ($null -ne $loadedRoot -and $null -ne $loadedRoot.PSObject.Properties['SourcePath']) {
        if (-not [IO.Path]::GetFullPath([string]$loadedRoot.SourcePath).Equals($dpPath, [StringComparison]::OrdinalIgnoreCase)) {
            throw "A different DirectPort substrate is already loaded: $($loadedRoot.SourcePath)"
        }
    } else {
        . $dpPath -Headless
    }

    if ($null -eq $global:DirectPort -or $null -eq $global:DirectPort.NewCanvas) {
        throw 'DirectPort.ps1 loaded, but its Canvas capability was not retained.'
    }
    $canvasFactory = $global:DirectPort.NewCanvas
    if ($null -ne $attachmentContext -and $null -ne $attachmentContext.Node) {
        ...
        if ($null -eq $canvasEntry) { throw 'The attached application node has no local Canvas capability.' }
    ...
    Wait-DirectPortFailureAcknowledge
    throw
}
...
function Start-Calculator([switch] $Headless) {
    ...
    try {
        ...
        $runtime = Get-Variable -Name DirectPortRuntimeState -Scope Global -ValueOnly -ErrorAction SilentlyContinue
        ...
        Write-DirectPortPanel @(
            ...
            " DirectPort  $dpPath "
            " Runtime     DirectPort $runtimeVersion ($runtimePlatform) "
        ...
        while ($gpu.Alive) {
            ...
            # Render only when DirectPort reports that the Canvas itself needs
